...
class MPCTensor(CrypTensor):
    ...
    # Logistic Functions
    @mode(Ptype.arithmetic)
    def sigmoid(self, method=None, terms=32):
        """Computes the sigmoid function using the following definition

        .. math::
            \sigma(x) = (1 + e^{-x})^{-1}

        If a valid method is given, this function will compute sigmoid using that method:

        "chebyshev" - computes tanh via Chebyshev approximation with truncation and uses the identity:

        .. math::
            \sigma(x) = \frac{1}{2}tanh(\frac{x}{2}) + \frac{1}{2}

        Args:
            terms (int): highest degree of Chebyshev polynomials for tanh using Chebyshev approximation.
    ...
    @mode(Ptype.arithmetic)
    def tanh(self, method=None, terms=32):
