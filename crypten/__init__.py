...
def _setup_prng():
    ...
    def generate_seeds():
        ...
        return prev_seed.item(), next_seed.item(), local_seed, global_seed
    ...
    for device in generators["prev"].keys():
        generators["prev"][device].manual_seed(seeds[0])
        generators["next"][device].manual_seed(seeds[1])
        generators["local"][device].manual_seed(seeds[2])
        generators["global"][device].manual_seed(seeds[3])
